...
class ApiService {
    ...
    /** @param {'thumbs_up'|'thumbs_down'} rating */
    ...
    async exportChatHistoryPdf(filename = 'career-plan.pdf') {
        const token = await getIdToken();
        const headers = {};
        if (token) headers['Authorization'] = `Bearer ${token}`;

        const response = await fetch(`${API_BASE_URL}/api/chat/export/pdf`, { headers });
        if (!response.ok) {
            const err = await response.json().catch(() => ({}));
    ...
    async exportSessionPdf(_sessionId, filename = 'career-plan.pdf') {
        return this.exportChatHistoryPdf(filename);
    }
